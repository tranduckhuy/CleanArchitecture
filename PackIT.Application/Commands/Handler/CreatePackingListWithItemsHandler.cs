...
namespace PackIT.Application.Commands.Handler
{
    public class CreatePackingListWithItemsHandler : ICommandHandler<CreatePackingListWithItems>
    {
        ...
        private readonly IWeatherSerivce _weatherSerivce;

        public CreatePackingListWithItemsHandler(IPackingListRepository repository, IPackingListFactory factory,
            IPackingListReadService readService, IWeatherSerivce weatherSerivce)
        {
            _repository = repository;
            _factory = factory;
            _readService = readService;
            _weatherSerivce = weatherSerivce;
        }
        ...
        public async Task HandleAsync(CreatePackingListWithItems command)
        {
            ...
            var weather = await _weatherSerivce.GetWeatherAsync(localization);
